...
data class Vec3(var x: Float = 0f, var y: Float = 0f, var z: Float = 0f) {
    ...
    operator fun times(vec: Vec3) = Vec3(x * vec.x, y * vec.y, z * vec.z)
...
fun distance(first: Vec3, second: Vec3) = length(first - second)
fun length(vec: Vec3) = sqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z)
...
fun clamp(vec: Vec3, toLength: Float) =
    if (length(vec) > toLength) normalize(vec) * toLength else vec